...
1. **Analyse.** Read the implementation. Omit `tilth_read` `mode`; the default shows small files in full and outlines large ones. Pass `mode: full` only when a section cannot answer the question. Inventory public functions, methods, and classes with `tilth_search` symbol and caller queries; map dependencies and integration points; plan the assault.
