...
#
# Strategy mirrors skills-external.bats: real jq/yq, fake external services
# (curl, systemctl, tar) on PATH. Healthcheck/install functions are sourced;
# both scripts are source-safe because main() runs only on direct execution.
...
setup() {
    ...
    # Symlink the mocks generated once in setup_file() so each test reuses the
    # target inode. Tests with bespoke curl/tar/uname mocks remove the symlink
    # before writing to avoid truncating the shared master.
    local f
...
@test ".chezmoiremove deletes the deployed retired units" {
    ...
    grep -q "local-llm/configs/lean.json" "$remove"
}
