...
def local_payload(values: tuple[Any, ...] = (10, 12, 14)) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "source": "local",
        "command": "just check",
...
class BoundaryTests(unittest.TestCase):
    def normalize(self, command: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def compare(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            values = (before, resolved_after)
            for path, value in zip(paths[:2], values, strict=True):
                path.write_text(json.dumps(value), encoding="utf-8")
            if ack is not None:
                paths[2].write_text(json.dumps(ack), encoding="utf-8")
            args = [
                "compare",
                "--before",
                str(paths[0]),
    ...
    def test_local_numeric_and_outcome_boundaries(self) -> None:
    ...
    def test_cancelled_codes_and_malformed_normalized_source(self) -> None:
    ...
    def test_unknown_context_cannot_be_acknowledged(self) -> None:
    ...
    def test_context_acknowledgement_is_exact(self) -> None:
    ...
    def test_mixed_sources_and_insufficient_samples_are_limits(self) -> None:
    ...
    def test_normalized_identity_and_eligibility_cannot_be_forged(self) -> None:
    ...
    def test_ci_timing_and_population_boundaries(self) -> None:
    ...
    def test_capture_completeness_cannot_be_forged(self) -> None:
        payload = ci_payload()
        ...
        for total in (0, 1):
            with self.subTest(total=total):
                ...
                self.assertIn("inconsistent_job_population", observation["reasons"])
    ...
    def test_large_finite_median_and_zero_baseline(self) -> None:
    ...
    def test_output_and_encoding_contract(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            source.write_text(json.dumps(payload), encoding="utf-8")
            result = run_cli("local", "--input", str(source), "--force")
            self.assertEqual(result.returncode, 2, result.stderr)
            self.assertIn("--force requires an output path", result.stderr)
