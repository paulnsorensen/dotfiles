...
cheese-flow's Copilot renderer writes project-scoped agent/skill/hook artifacts into a `.github/` layout. It does not mutate live `~/.copilot/mcp-config.json`; durable MCP defaults belong to the chezmoi template (`private_dot_copilot/mcp-config.json.tmpl`, env-rendered API keys) or Copilot's own runtime state.
...
| MCP | <https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-mcp-servers> | Chezmoi template / user-owned `~/.copilot/mcp-config.json` (`mcpServers` schema; stdio/HTTP/SSE). `agents/mcp/registry.yaml` feeds compiled profile payloads where the renderer owns the target; `cheese profile compile/apply` does not own this live file. |
...
No closed-world isolated settings are available. The wrapper delegates ordinary invocation to `cheese profile launch copilot global --source-root "$DOTFILES_DIR" -- "$@"`.

## Quirks

- Skill/agent/hook artifacts target Copilot's **project-scoped** `.github/` layout (its read convention), unlike the other harnesses' user-scoped `~/.<harness>/` deploys. Profile `live` compiles with `target_root: $HOME`, so its Copilot renderer writes `.github/…` beneath `$HOME` (`~/.github/`), not the current repo.
