...
Root `dots sync` verifies exact live outputs — currently `omp/18.0.5` (`OMP_PIN` in `packages/sync.sh`, renovate-locked in lockstep with the `.sync` guard literal; see [[../operations/sync-and-chezmoi]]) and `codex-cli 0.146.0` — after package convergence, applies the final chezmoi state under those schemas, and repeats both exact probes after a successful final apply. A failed final apply skips post-apply probes; a failed post-apply probe reports `harness-versions` while retaining upgraded binaries. The focused ordering, mismatch, absence, command-failure, and upgraded-state-retention tests live at `tests/sync-orchestrator.bats:180-548`.
