...
OMP 18.1.14 uses Milknado as its sole work tracker. Native Todo and its reminders are disabled in the repo-authoritative chezmoi data, the system prompt assigns planning to Milknado MCP, and a directly discovered input extension consumes `/todo` before OMP can create a disconnected native list.
...
Instead OMP deploys native extensions under `~/.omp/agent/extensions/`: `cheese-flair.ts`, `milknado-todo-guard.ts`, `commit-hallouminate-reminder.ts`, and `sliced-bread-audit.ts`. The flair extension runs the deployed `~/.claude/hooks/session-start-cheese-flair.sh` script, so both harnesses share one flair source.[^3]
...
[^3]: `chezmoi/dot_omp/private_agent/extensions/`
...
Root `dots sync` verifies exact live outputs — currently `omp/18.2.8` (`OMP_PIN` in `packages/sync.sh`, renovate-locked in lockstep with the `.sync` guard literal; see [[../operations/sync-and-chezmoi]]), `codex-cli 0.154.0`, and Pi `0.87.0` — after package convergence, applies the final chezmoi state under those schemas, and repeats all three exact probes after a successful final apply. A failed final apply skips post-apply probes; a failed post-apply probe reports `harness-versions` while retaining upgraded binaries. The focused ordering, mismatch, absence, command-failure, and upgraded-state-retention tests live at `tests/sync-orchestrator.bats:180-548`.
