...
|---|---|---|
| Hooks | <https://developers.openai.com/codex/hooks> | `agents/hooks/registry.yaml` → `~/.codex/hooks.json`. `matcher` (e.g. `startup\|resume\|clear`) is codex-only for SessionStart. `timeout` in seconds. Codex 0.148.0+ runs `async: true` command hooks in the background (up to 8 concurrent per session) — no longer ignores the field; see the `async` field note in the registry for what this repo currently wires. |
...
- **codex-cli pin (2026-09-10 doc-drift pass)**: `chezmoi/dot_config/mise/config.toml` bumped `aqua:openai/codex` from `rust-v0.153.4` to `rust-v0.154.0`. Checked the [rust-v0.154.0 release](https://github.com/openai/codex/releases/tag/rust-v0.154.0) against all four governed surfaces (this doc, `agents/hooks/registry.yaml`, `agents/registry.yaml`, the mise pin): headline items (GPT-6-Astra model/Bedrock catalog entries, experimental `--worktree`/`/worktree` session isolation, inline question answering, a Windows daemon server, Vim replace mode, richer `/copy`) are all product-surface, not config/schema, and none of the release's bug fixes touch the hook event/matcher/timeout schema or the per-file `~/.codex/agents/<n>.toml` schema (`name`/`description`/`developer_instructions` + optional overrides). One deprecation worth flagging: **the `codex mcp-server` entry point was removed** in this release; a repo-wide `grep -ri mcp-server` confirms nothing here ever invoked it (the only hits are unrelated — Serena's own `start-mcp-server` arg, Copilot's `mcp-config.json`, and doc/test examples), so this is a pure no-op for us. The open roles/multi-agent-V2 design question tracked in [dotfiles#503](https://github.com/paulnsorensen/dotfiles/issues/503) is unaffected — 0.154.0 adds nothing to that surface.
- **codex-cli pin (2026-08-30 doc-drift pass)**: `chezmoi/dot_config/mise/config.toml` bumped `aqua:openai/codex` from `rust-v0.146.0` to `rust-v0.151.0`. Surveyed the intervening releases (0.147.0–0.151.0) against this repo's governed Codex surfaces (hooks registry, sub-agent registry, this doc, the mise pin): the only change found to reach anything this repo configures is background/async hooks, called out above and in `agents/hooks/registry.yaml`'s `async` field comment. Everything else in that range (Bedrock provider, `codex agents`/`codex doctor`/`codex queue` commands, thread credits, session fork/export/archive, `max`/`ultra` SDK reasoning effort, MCP tool-result rewriting extensions) is either a product-surface change with no config/doc footprint here, or (the `Interrupt` hook event some third-party changelogs attribute to 0.150.0) not corroborated by the official hooks doc's event list as of this pass — not added on that basis.
