...
- `~/.claude/{skills,agents,commands,hooks,lib,reference,workflows}` — deployed as chezmoi **`exact_` dirs**, assembled at sync time by `sync_claude_chezmoi_sources` (`.sync-lib.sh`, called from `chezmoi/.sync` before apply): registry-selected local skills (`claude.yaml skills:`), vendored external skills (`skills/_registry.yaml` → offline-tolerant clone cache under `~/.cache/dotfiles/claude-skill-sources`), agents rendered with frontmatter from `agents/registry.yaml`, and the `claude/` + `agents/{hooks,lib,reference}` asset dirs. `exact_` semantics = **deletions propagate**. The assembled trees are derived, gitignored state; repo dirs stay the source of truth. After a successful apply, `bin/dots:do_sync` forces `chezmoi/lib/install-external.sh` for Cursor only. It sets `SKILL_EXCLUDE_AGENTS=claude-code` and refuses `CHEZMOI_OWNED_AGENTS`/`CHEZMOI_OWNED_HARNESSES` (codex, github-copilot, zed, omp). The installer compares the source checkout with Cursor's `skills list --json` output and removes source-owned names that disappeared upstream.
- `~/.agents/skills` — deployed as a chezmoi **`exact_` dir**, assembled at sync time by `sync_shared_agents_chezmoi_sources` (`.sync-lib.sh`): the union of the local `.claude.skills` selection and vendored external sources filtered on harnesses `codex copilot zed omp`, plus plugin sources. Codex, Copilot, Zed, and OMP's `agents` provider all read this one dir; the shared dir is chezmoi-owned. `.chezmoiremove` drops the two roots it replaces: `~/.github/skills` (Copilot's second root) and `~/.omp/agent/skills` (OMP's retired native root).
