...
- `~/.pi/agent/` — first-class upstream Pi state from `pi.yaml` + `dot_pi/`: wholesale `settings.json`, templated `models.json`, managed MCP/package files, native addendum, and exact skills/extensions/themes assembled by `.sync-lib.sh`. Runtime auth, trust, and session stores remain outside the managed tree.
- Global instructions (`~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.pi/agent/AGENTS.md`) and Codex config are installed by the `run_onchange_*` helpers.
