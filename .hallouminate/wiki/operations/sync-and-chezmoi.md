...
- `~/.claude/settings.json` — `dot_claude/modify_settings.json` rewrites the live file from `chezmoi/lib/claude-settings-authoritative.json` on every apply. Repo-owned keys win; app/renderer-owned live keys (`permissions.allow`/`deny`/`ask`/`additionalDirectories`, `enabledPlugins`, `extraKnownMarketplaces`) are preserved from live or intentionally asserted here; unknown live key paths fail the apply so new Claude Code settings get classified instead of silently clobbered. See [[claude-dotfiles-ownership]].
...
- `~/.claude/skills/`, `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, prompt files, and generated agent artifacts — handled by `run_onchange_*` scripts under `chezmoi/.chezmoiscripts/` that fork to helpers in `chezmoi/lib/` and/or `ap`.
- `~/.claude/{commands,hooks,reference,workflows}` — deployed as **one-way copies** by `install-claude-assets.sh` (via `run_onchange_after_install-claude-assets.sh.tmpl`), manifest-tracked under `.dotfiles-managed-claude-assets` (drops repo-removed items, preserves user files, self-migrates a legacy symlink target to a real dir). These used to be whole-dir symlinks from `claude/.sync`; copy stops runtime writes leaking back into the repo (same class of bug as the `~/.cursor` symlink). Trade-off: edits need a `dots sync`/`chezmoi apply` to go live. Source stays editable in `claude/` (not relocated into `chezmoi/`). `claude/hooks/` is pure JS — the dead wink-nlp `npm install` + `node_modules` were removed.
