...
- `~/.pi/agent/` — `pi.yaml` and `dot_pi/` own settings, models, MCP configuration, package policy, extensions, and theme. Pi keeps auth, trust, sessions, caches, and package stores. Pi discovers shared skills from `~/.agents/skills`; chezmoi does not copy them.
- Global instructions for Claude, Codex, and Pi, plus Codex config, are installed by the `run_onchange_*` helpers.
...
on rules their upstream repos never enforced. The OMP tree was missed until
2026-09-06 (PR #893); Claude and Codex were already listed.
