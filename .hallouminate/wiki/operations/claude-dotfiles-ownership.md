...
- Registry-authored keys (`permissions.allow`, `permissions.deny`, `permissions.ask`, `permissions.additionalDirectories`, `enabledPlugins`, `extraKnownMarketplaces`) are composed wholesale from source, NOT preserved from the live file: `chezmoi/.chezmoidata/claude.yaml` base + `claude/plugins/registry.yaml` (gate-filtered overlay) + `agents/plugins/registry.yaml` (native-claude overlay — `native: ∋ claude` entries, keyed by each marketplace.json `.name`). Live drift in the `enabledPlugins` / `extraKnownMarketplaces` subtrees is wiped on apply, not halted on. `modify_settings.json` is the single writer; `ap` no longer renders these globally (its `_render_native_plugins` returns early unless the profile is `isolated`, `agent-profile/agent_profile/renderers/claude.py`). The CLI-side marketplace index + `installed_plugins.json` are primed/pruned by `chezmoi/.chezmoiscripts/run_onchange_after_sync-claude-plugins.sh.tmpl` + `chezmoi/lib/claude-plugin-reconcile.sh` (manifest-owned prune only).
