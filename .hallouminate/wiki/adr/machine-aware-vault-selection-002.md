...
Serialize resolver and materializer cache transactions with one persistent mode-0600 advisory-lock file. On macOS, BSD `lockf -k` locks the pathname while a fresh Bash process sources the vault library and runs the complete callback. On Linux, including BusyBox, a bounded non-blocking `flock` loop locks descriptor 9 around the callback. The file is never unlinked, so every contender addresses the same inode; kernel locking replaces PID metadata and stale-lock reaping.
