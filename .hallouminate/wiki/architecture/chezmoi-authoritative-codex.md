...
The legacy `~/.codex/skills` mirror is now evicted wholesale on every `dots
sync`. The current `skills` CLI installs the codex agent into the shared
`~/.agents/skills`, so that root only held frozen copies that Codex still read —
mixing old and new skill instructions (age v0.5.3 / ten dimensions, cook with
`cheez-write`, retired ultracook). `chezmoi/lib/install-external.sh`
(`evict_legacy_codex_skills`) removes the whole tree; `tests/skills-external.bats`
covers it. This supersedes the earlier decision to leave it frozen.

Still deliberately excluded: `rules/` (generated by `ap` from permission data).
Cursor and Copilot global settings remain disconnected from automatic writes.

See also [[codex-first-class-review]], [[../harnesses/codex-hooks-schema]],
[[../operations/sync-and-chezmoi]], [[config-drift]].
