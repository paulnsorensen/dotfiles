...
- **External**: `_registry.yaml` sources are fetched by `npx skills add` for CLI-supported harnesses and vendored by the chezmoi assembler for the shared `~/.agents/skills` dir (`sync_shared_agents_chezmoi_sources`), honoring each source's `harnesses:` filter.
