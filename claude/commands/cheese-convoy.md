...
Agent(
  ...
  prompt="Use the Skill tool to invoke skill='move-my-cheese' with args='<PR#>'. This runs the full PR rescue workflow. You are running in a worktree alongside other parallel agents — direct LSP calls are disallowed; navigate with tilth_search (kind: symbol | callers | regex), tilth_deps, and tilth_read(paths: [...]) batches. If a diagnosis genuinely needs type inference, escalate via `/explore` (cheese-flow:explore-lsp). After the skill completes, report: what was wrong, what was fixed, quality sweep findings, and CI status."
)
```
