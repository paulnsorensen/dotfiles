...
- **Aspect D**: **External prior art** — invoke the `/research` skill (not `fromage-culture`) to scan how other projects solved similar problems. The skill routes across Context7, Octocode, Serper, and Tavily automatically. Pass `--report $TMPDIR/fromage-culture-<slug>-prior-art.md` so the findings file is created.
