...
local notes and run `/research` when a task's scope needs a fact check.
...
- **Bash** — available for `gh`, shell utilities, and `/research`'s gh CLI fetcher
- **/research skill** — multi-source research via `mcp__context7__*`, `mcp__tavily__*`, `mcp__serper__*`; use when a task needs a fact check or library doc lookup before it can be scoped
- **Agent** — spawns todoist-flow sub-agents (fetch, distill, scribe, qa) and `/research` fetchers
...
6. Use `/research` only when the task requires external facts or library docs — not for general browsing
