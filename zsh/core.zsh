...
# command execution). Cache wins on collision. A missing cache warns once but
# never breaks the shell (unlike agents/mcp/sync.sh and cc-env-exec, which
# fail loud — a fresh terminal must not brick on a vault hiccup).
...
[[ -f "$_dotfiles_cache_env" ]] || echo "dotfiles: vault secrets cache missing ($_dotfiles_cache_env) — run 'dots sync'" >&2
