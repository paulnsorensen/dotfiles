...
SCHEME_AUTHOR=$(yq -r '.author' "$SCHEME_FILE")
...
# ---------------------------------------------------------------------------
# Generate tmux/theme.conf
# ---------------------------------------------------------------------------
generate_tmux_theme() {
  local out="$REPO_DIR/tmux/theme.conf"
  mkdir -p "$REPO_DIR/tmux"

  # Palette mapping to catppuccin @thm_* variables.
  # catppuccin/tmux v2 exposes these as overridable user options; we pre-load
  # them before TPM runs so the plugin inherits our palette.
  #
  # Mapping logic:
  #   bg/fg/surface*/overlay*  <- base00-05 (background hierarchy + fg)
  #   accent colours           <- nearest-fit base0x slots
  #   @catppuccin_flavor        <- mocha (suppresses flavour palette; overrides win)
  cat > "$out" <<TMUX_THEME
# ${SCHEME_NAME} - tmux theme (catppuccin-driven, palette from theme/schemes/${ACTIVE}.yaml)
# Generated by theme/generate.sh — DO NOT EDIT

# ---------------------------------------------------------------------------
# Palette overrides: pre-load @thm_* before catppuccin/tmux initialises so
# our scheme colours take precedence over the built-in flavour definitions.
# ---------------------------------------------------------------------------
# Backgrounds (catppuccin surface hierarchy)
...
set -g @thm_peach       "#$(pal base09)"
set -g @thm_yellow      "#$(pal base0A)"
set -g @thm_green       "#$(pal base0B)"
set -g @thm_teal        "#$(pal base0C)"
set -g @thm_sky         "#$(pal base0C)"
set -g @thm_sapphire    "#$(pal base0D)"
set -g @thm_blue        "#$(pal base0D)"
set -g @thm_lavender    "#$(pal base0E)"
set -g @thm_mauve       "#$(pal base0E)"
set -g @thm_pink        "#$(pal base17)"
set -g @thm_flamingo    "#$(pal base12)"
set -g @thm_rosewater   "#$(pal base09)"

# ---------------------------------------------------------------------------
# catppuccin/tmux v2 config
# ---------------------------------------------------------------------------
# Use mocha as base flavour; @thm_* overrides above replace all colours.
...
# Status-right: clock only (continuum appends its save hook after TPM loads)
...
# ---------------------------------------------------------------------------
# Generate ghostty/config (for cmux)
# ---------------------------------------------------------------------------
generate_ghostty_config() {
  local out="$REPO_DIR/ghostty/config"
  mkdir -p "$REPO_DIR/ghostty"

  cat > "$out" <<GHOSTTY
# ${SCHEME_NAME} - Ghostty/cmux configuration
# Generated by theme/generate.sh from theme/schemes/${ACTIVE}.yaml
# DO NOT EDIT - changes will be overwritten on next \`dots sync\`

# Font (matches iTerm2 profile)
...
palette = 7=#$(pal base05)
palette = 8=#$(pal base03)
palette = 9=#$(pal base12)
palette = 10=#$(pal base14)
palette = 11=#$(pal base09)
palette = 12=#$(pal base16)
palette = 13=#$(pal base17)
palette = 14=#$(pal base15)
...
# ---------------------------------------------------------------------------
# Generate Zed theme (chezmoi-managed user theme file)
# ---------------------------------------------------------------------------
generate_zed_theme() {
  local out_dir="$REPO_DIR/chezmoi/dot_config/zed/themes"
  local out="$out_dir/dotfiles-theme.json"
  mkdir -p "$out_dir"
  ...
  cat > "$out" <<ZED_THEME
{
  ...
  "name": "${SCHEME_NAME}",
  "author": "${SCHEME_AUTHOR} (generated by theme/generate.sh from theme/schemes/${ACTIVE}.yaml — DO NOT EDIT)",
  "themes": [
    {
      "name": "${SCHEME_NAME}",
      "appearance": "dark",
      "style": {
        "background": "#$(pal base00)",
        "editor.background": "#$(pal base00)",
        "surface.background": "#$(pal base01)",
        "panel.background": "#$(pal base01)",
        "elevated_surface.background": "#$(pal base01)",
        "element.hover": "#$(pal base02)",
        "border": "#$(pal base02)",
        "border.variant": "#$(pal base01)",
        "editor.gutter.background": "#$(pal base00)",
        "text": "#$(pal base05)",
        "text.muted": "#$(pal base04)",
        "text.disabled": "#$(pal base03)",
        "text.placeholder": "#$(pal base03)",
        "editor.foreground": "#$(pal base05)",
        "editor.line_number": "#$(pal base03)",
        "editor.active_line_number": "#$(pal base0A)",
        "editor.active_line.background": "#$(pal base01)",
        "error": "#$(pal base08)",
        "warning": "#$(pal base0A)",
        "success": "#$(pal base0B)",
        "info": "#$(pal base0C)",
        "terminal.background": "#$(pal base00)",
        "terminal.foreground": "#$(pal base05)",
        "terminal.ansi.black": "#$(pal base01)",
        "terminal.ansi.red": "#$(pal base08)",
        "terminal.ansi.green": "#$(pal base0B)",
        "terminal.ansi.yellow": "#$(pal base0A)",
        "terminal.ansi.blue": "#$(pal base0D)",
        "terminal.ansi.magenta": "#$(pal base0E)",
        "terminal.ansi.cyan": "#$(pal base0C)",
        "terminal.ansi.white": "#$(pal base05)",
        "terminal.ansi.bright_black": "#$(pal base03)",
        "terminal.ansi.bright_red": "#$(pal base12)",
        "terminal.ansi.bright_green": "#$(pal base14)",
        "terminal.ansi.bright_yellow": "#$(pal base09)",
        "terminal.ansi.bright_blue": "#$(pal base16)",
        "terminal.ansi.bright_magenta": "#$(pal base17)",
        "terminal.ansi.bright_cyan": "#$(pal base15)",
        "terminal.ansi.bright_white": "#$(pal base07)",
        "syntax": {
          "comment": { "color": "#$(pal base03)" },
          "string": { "color": "#$(pal base0B)" },
          "keyword": { "color": "#$(pal base0E)" },
          "function": { "color": "#$(pal base0D)" },
          "type": { "color": "#$(pal base0A)" },
          "number": { "color": "#$(pal base09)" },
          "constant": { "color": "#$(pal base09)" },
          "boolean": { "color": "#$(pal base09)" },
          "variable": { "color": "#$(pal base05)" },
          "operator": { "color": "#$(pal base0E)" },
          "punctuation": { "color": "#$(pal base05)" },
          "property": { "color": "#$(pal base0D)" },
          "tag": { "color": "#$(pal base08)" },
          "embedded": { "color": "#$(pal base05)" }
        },
        "players": [
          { "cursor": "#$(pal base0D)", "background": "#$(pal base0D)", "selection": "#$(pal base0D)3d" },
          { "cursor": "#$(pal base0B)", "background": "#$(pal base0B)", "selection": "#$(pal base0B)3d" },
          { "cursor": "#$(pal base0A)", "background": "#$(pal base0A)", "selection": "#$(pal base0A)3d" },
          { "cursor": "#$(pal base0E)", "background": "#$(pal base0E)", "selection": "#$(pal base0E)3d" },
          { "cursor": "#$(pal base0C)", "background": "#$(pal base0C)", "selection": "#$(pal base0C)3d" },
          { "cursor": "#$(pal base08)", "background": "#$(pal base08)", "selection": "#$(pal base08)3d" },
          { "cursor": "#$(pal base09)", "background": "#$(pal base09)", "selection": "#$(pal base09)3d" },
          { "cursor": "#$(pal base17)", "background": "#$(pal base17)", "selection": "#$(pal base17)3d" }
        ]
      }
    }
  ]
