...
- **Decision:** Use Node's builtin `node:test` runner (`tests/workflows/*.test.mjs`, invoked via a thin `tests/workflows-test.sh` that skips when node is absent) and a ~60-line subset validator covering `type/required/properties/items/enum` — the only keywords the 9 scripts use — which throws on any unknown keyword.
