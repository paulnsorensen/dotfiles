...
ALL_HARNESSES = list(SUPPORTED_ITEM_HARNESSES)
_HARNESS_CHOICES = "|".join(ALL_HARNESSES)
...
def _fetch_external_skills(
    ...
    *,
    ...
    skill_harnesses = [h for h in harnesses if h in ALL_HARNESSES]
    ...
    for h in harnesses:
        if h not in ALL_HARNESSES:
            raise CliError(
                ...
                f"(valid: {_HARNESS_CHOICES}){colors.NC}"
            )
