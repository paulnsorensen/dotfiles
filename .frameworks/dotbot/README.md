...
`dotbot-brew` was evaluated but not adopted in this foundation yet. The current package surface spans Homebrew, cargo, npm, uv, and Linux apt checks, so `/home/runner/work/dotfiles/dotfiles/packages/sync.sh` remains the single integration point for now.
