---
name: grok-codebase
description: Use when the user wants to map, explore, summarize, memorize, or "grok" an unfamiliar codebase. Triggers on phrases like "help me understand this repo", "grok this", "onboard me", "give me a read-only tour", "map the architecture", "memorize this project", "walk me through this code", "explain this project to me", "quiz me on this repo". Loads the codebase into long-term understanding using a four-pillar model — Building Blocks → Entry Points → Infrastructure → Egress — and finishes with an adaptive Socratic quiz. Read-only stance — DO NOT propose edits unless explicitly asked. Especially tuned for TypeScript/JavaScript monorepos and Next.js / Express / NestJS / Fastify apps; methodology is stack-agnostic. Do NOT use for single-file scripts, repos under 500 LOC, or editing tasks.
