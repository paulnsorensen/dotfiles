...
- Tier bindings for delegation: `powerful` = `@strong` (GPT-5.6 Sol; `plan`, `default`), `default` = `@balanced` (Terra), `cheap` = `@fast` (Luna; `task`, `tiny`, `smol`). Agent files pin the alias: `reviewer` and `cheese-reviewer` `@strong`, `coder` `@fast`. A taste-test runs on the pinned `reviewer`.
