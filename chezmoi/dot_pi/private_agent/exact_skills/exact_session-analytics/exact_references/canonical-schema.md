...
(`claude`/`codex`/`omp`/`cursor`/`copilot`). Filter or group by it to compare
sources; omit it to aggregate across all reachable harnesses.
...
System entries with subtype `stop_hook_summary` (Claude only; Codex and OMP
