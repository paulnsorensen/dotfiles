...
description: >
  Query Claude, Codex, OMP, and upstream Pi coding-agent session logs via DuckDB
  for usage analytics, tool patterns, error forensics, and routing decisions.
  Use when the user says "session analytics", "query my logs", "tool usage", "how often do I
...
Session logs come from Claude JSONL, Codex rollout JSONL, and OMP JSONL.
`ingest.py` normalizes each source into one canonical row shape with a
`harness` column, then materializes pre-flattened tables. Cursor and Copilot
have no accessible logs today (see `harness-coverage.md`).
...
column so you can compare Claude, Codex, and OMP.
