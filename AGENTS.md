...
- **Per-directory .sync scripts** for custom setup (fonts, iterm2, chezmoi, .copilot)
...
**Chezmoi wiring**: `chezmoi/.sync` writes `~/.config/chezmoi/chezmoi.toml` with `sourceDir = <absolute-path-to-dotfiles>/chezmoi` exactly once, on first setup. After that the file is never rewritten — chezmoi owns it, and user edits to any key (`sourceDir`, `[diff]`, `[merge]`, etc.) survive every subsequent `dots sync`. The write is atomic (`mktemp` + `mv -f`), so a SIGINT mid-sync cannot corrupt the config. Direct `chezmoi <cmd>` invocations and `dots sync` (which calls `chezmoi apply` after the wiring step) both resolve to the same source dir via the absolute path in `chezmoi.toml`. `chezmoi/` stays out of `SYNC_SKIP_LIST` because the `.sync` script bypasses the default symlink path anyway.

**Chezmoi migration plan**: The current `dots` + `.sync-with-rollback` stack is being phased out in favor of chezmoi as the canonical sync tool. Phase 0 (this scaffold) wires chezmoi alongside the existing flow without managing any files yet (`chezmoi/.chezmoiignore` is `*`). The full multi-phase roadmap lives in `.cheese/specs/chezmoi-migration.md` — read it before extending the chezmoi side of the sync.
...
- Pre-tool hooks: `bash-guard.js`, `phantom-file-check.js`, `write-guard.js`, `review-reply-guard.js` (`worktree-guard.js` exists but is currently disengaged)
