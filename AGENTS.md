...
**Ownership** is tracked by a single machine-level, ref-counted manifest at `~/.cursor/.dotfiles-cursor-manifest.json` (`{ "<plugin>": { "files": [...] } }`) — this **replaces** the old per-dir `.dotfiles-managed-<plugin>` markers. Whole-file/dir artifacts (skills, rules, commands, hook scripts) are recorded as relpaths; a path dropped from a plugin's source is removed on the next sync **only when no other plugin still claims it** (ref-count). The manifest validates on read — corruption fails the deploy loud. Hook + mode entries in `hooks.json` / `modes.json` are instead tagged with `_plugin: "<name>"` so re-deploys can strip stale entries without touching unrelated ones; user-authored files at every target are preserved.

**Three safety guards** in `install-cursor-plugin.sh`:

- **Target guard** — the script dies loud and writes nothing if the deploy target (`CURSOR_HOME`) is at or inside the dotfiles repo root (the script anchors on its own repo). This is what prevents the stray-artifact class where a mis-pointed deploy lands in `dotfiles/cursor/` instead of `~/.cursor`.
- **Merge integrity** — `hooks.json` / `modes.json` merges go through a `validated_mv` (PR #188 pattern, mirroring the codex round-trip guard in `agents/hooks/lib.sh`): it refuses the `mv` if the merge would drop any top-level key present in the destination, even when the jq itself succeeds.
- **Collision guard** — a deploy that would land on a `~/.cursor/skills` name owned by neither this plugin nor any other dotfiles plugin (i.e. `gh`-installed or hand-authored) warns loudly and skips that item, non-fatal, so `dots sync` continues. cheese-grok and `gh` externals must hold disjoint skill names.

The deploy outputs in repo `cursor/` (`commands/`, `hooks/`, `rules/`, `skills/`, `hooks.json`, `mcp.json`, `modes.json`) are gitignored; only `cursor/plugins/local/` is tracked source. For the full model — the three config channels, the machine-vs-project (`~/.cursor` vs `<repo>/.cursor` via `ap`) split, and the relationship to PR #181 — see the `/cursor` skill.
