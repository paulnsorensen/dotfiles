...
**Chezmoi wiring**: `chezmoi/.sync` owns the wiring — it creates the `~/.chezmoi → dotfiles/chezmoi/` symlink and writes `~/.config/chezmoi/chezmoi.toml` with `sourceDir = ~/.chezmoi`. Direct `chezmoi <cmd>` invocations and `dots sync` (which calls `chezmoi apply` without `--source`) both resolve to the same source dir. The chezmoi.toml write is idempotent: if the sourceDir line is already correct, the file is left untouched so user-added keys (e.g. `[diff]`, `[merge]`) survive across syncs. `chezmoi/` stays out of `SYNC_SKIP_LIST` because the `.sync` script bypasses the default symlink path anyway.
